...
class LenderRepository(BaseRepository[Lender]):
    ...
    async def get_by_code(
        self, organization_id: UUID, lender_code: str
    ) -> Optional[Lender]:
        """Get lender by code."""
    ...
    async def generate_lender_code(self, organization_id: UUID) -> str:
        ...
        query = select(func.count()).select_from(Lender).where(
            Lender.organization_id == organization_id
        )
        result = await self.session.execute(query)
...
class BorrowingRepository(BaseRepository[Borrowing]):
    ...
    async def get_by_number(
        self, organization_id: UUID, borrowing_number: str
    ) -> Optional[Borrowing]:
    ...
    async def get_with_details(self, borrowing_id: UUID) -> Optional[Borrowing]:
    ...
    async def generate_borrowing_number(
        self, organization_id: UUID, borrowing_type: str
    ) -> str:
        """Generate next borrowing number."""
        year = date.today().year
        query = select(func.count()).select_from(Borrowing).where(
            and_(
                Borrowing.organization_id == organization_id,
...
class BorrowingTrancheRepository(BaseRepository[BorrowingTranche]):
    ...
    async def get_by_borrowing(
        ...
        query = select(BorrowingTranche).where(
...
class BorrowingScheduleRepository(BaseRepository[BorrowingSchedule]):
    ...
    async def get_by_borrowing(
        ...
        query = select(BorrowingSchedule).where(
...
class BorrowingPaymentRepository(BaseRepository[BorrowingPayment]):
    ...
    async def get_by_borrowing(
        ...
        query = select(BorrowingPayment).where(
            BorrowingPayment.borrowing_id == borrowing_id
        ).order_by(BorrowingPayment.payment_date.desc())
        return await self._paginate(query, skip, limit)

    async def get_total_paid(
        self, borrowing_id: UUID, payment_type: Optional[str] = None
    ) -> Decimal:
...
class ALMPositionRepository(BaseRepository[ALMPosition]):
    ...
    async def get_by_date(
        self, organization_id: UUID, position_date: date
    ...
    async def get_latest(self, organization_id: UUID) -> Optional[ALMPosition]:
    ...
    async def get_with_details(self, position_id: UUID) -> Optional[ALMPosition]:
...
class ALMAssetRepository(BaseRepository[ALMAsset]):
    ...
    async def get_by_position(self, position_id: UUID) -> List[ALMAsset]:
        ...
        query = select(ALMAsset).where(
            ALMAsset.position_id == position_id
...
class ALMLiabilityRepository(BaseRepository[ALMLiability]):
    ...
    async def get_by_position(self, position_id: UUID) -> List[ALMLiability]:
        ...
        query = select(ALMLiability).where(
            ALMLiability.position_id == position_id
...
class ExposureTrackingRepository(BaseRepository[ExposureTracking]):
    ...
    async def get_by_limit(
        ...
        query = select(ExposureTracking).where(
            ExposureTracking.limit_id == limit_id
        ).order_by(ExposureTracking.exposure_amount.desc())
        return await self._paginate(query, skip, limit)

    async def get_by_entity(
        self, entity_id: UUID
    ) -> List[ExposureTracking]:
        """Get exposure tracking records by entity."""
        query = select(ExposureTracking).where(
            ExposureTracking.entity_id == entity_id
        )
        result = await self.session.execute(query)
        return list(result.scalars().all())
