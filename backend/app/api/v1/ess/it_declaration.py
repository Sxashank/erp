...
async def add_hra_receipt(
    ...
    receipt = await service.add_hra_receipt(
        declaration_id=declaration_id,
        **request.model_dump()
    )
...
async def update_home_loan_details(
    ...
    try:
        declaration = await service.update_home_loan_details(
            declaration_id=declaration_id,
            **request.model_dump()
        )
    except ValueError as e:
...
def _format_declaration_detail(declaration) -> DeclarationDetailResponse:
    ...
    return DeclarationDetailResponse(
        ...
        rent_paid_monthly=float(declaration.rent_paid_monthly) if declaration.rent_paid_monthly else None,
        landlord_name=declaration.landlord_name,
        metro_city=declaration.metro_city,
        home_loan_interest=float(declaration.home_loan_interest) if declaration.home_loan_interest else None,
        home_loan_principal=float(declaration.home_loan_principal) if declaration.home_loan_principal else None,
        property_type=declaration.property_type,
        estimated_taxable_income=float(declaration.estimated_taxable_income) if declaration.estimated_taxable_income else None,
        estimated_tax_liability=float(declaration.estimated_tax_liability) if declaration.estimated_tax_liability else None,
