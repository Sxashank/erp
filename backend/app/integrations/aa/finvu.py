"""Finvu Account Aggregator client implementation."""
...
class FinvuAAClient(AAClientBase):
    """Finvu Account Aggregator client.

    Implements FIU APIs for Finvu AA platform.
    Documentation: https://docs.finvu.in/
    """
    ...
    def __init__(self, config: Dict[str, Any], sandbox_mode: bool = True):
        ...
        # Finvu-specific URLs
        if sandbox_mode:
            self.base_url = config.get("sandbox_url", "https://fiu-uat.finvu.in/api")
            self.token_url = config.get("sandbox_token_url", "https://fiu-uat.finvu.in/oauth/token")
        else:
            self.base_url = config.get("api_base_url", "https://fiu.finvu.in/api")
            self.token_url = config.get("token_url", "https://fiu.finvu.in/oauth/token")
    ...
    async def get_access_token(self) -> str:
        """Get or refresh OAuth access token from Finvu."""
        ...
        try:
            async with httpx.AsyncClient(timeout=30.0) as client:
                ...
                if response.status_code == 200:
                    ...
                    logger.error(f"Failed to get Finvu access token: {response.text}")
                    raise Exception(f"Token request failed: {response.status_code}")

        except Exception as e:
            logger.error(f"Finvu token error: {e}")
            raise
    ...
    async def create_consent(
        ...
        """Create a consent request with Finvu."""
        try:
            ...
            # Build consent request as per AA 2.0 spec
            consent_start = datetime.utcnow()
            consent_expiry = consent_start + timedelta(days=request.consent_validity_months * 30)
            data_life_expiry = consent_start + timedelta(days=request.data_life_months * 30)
            ...
            async with httpx.AsyncClient(timeout=30.0) as client:
                ...
                if response.status_code in (200, 201) and data.get("ver"):
                    ...
                    return AAConsentResponse(
                        ...
                        error_message=error_info.get("errorMessage", data.get("message")),
                        raw_response=data,
                    )

        except Exception as e:
            logger.error(f"Finvu create consent error: {e}")
            return AAConsentResponse(
                success=False,
                error_message=str(e),
            )
    ...
    async def get_consent_status(
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=30.0) as client:
                ...
                if response.status_code == 200:
                    ...
                    return AAConsentResponse(
                        ...
                        consent_id=consent_status.get("id"),
            ...
            logger.error(f"Finvu consent status error: {e}")
            return AAConsentResponse(
                success=False,
                consent_handle=consent_handle,
                error_message=str(e),
    ...
    async def fetch_consent_artifact(
        ...
        try:
            ...
            logger.error(f"Finvu fetch consent artifact error: {e}")
            return {}
    ...
    async def initiate_fi_request(
        ...
        try:
            ...
            if request.data_range_from:
                data_range["from"] = request.data_range_from.isoformat() + "T00:00:00.000Z"
            if request.data_range_to:
                data_range["to"] = request.data_range_to.isoformat() + "T23:59:59.999Z"

            payload = {
                "ver": "2.0.0",
                "timestamp": datetime.utcnow().isoformat() + "Z",
                "txnid": txn_id,
                "FIDataRange": data_range if data_range else None,
                "Consent": {
                    "id": request.consent_id,
                    "digitalSignature": "",  # Will be signed by AA
                },
                "KeyMaterial": {
                    "cryptoAlg": "ECDH",
                    "curve": "Curve25519",
            ...
            logger.error(f"Finvu FI request error: {e}")
            return AAFetchResponse(
                success=False,
                error_message=str(e),
            )
    ...
    async def fetch_fi_data(
        ...
        try:
            ...
            logger.error(f"Finvu fetch FI data error: {e}")
            return AAFetchResponse(
                success=False,
                session_id=session_id,
                error_message=str(e),
    ...
    async def decrypt_fi_data(
        ...
        Args:
            encrypted_data: Contains encryptedFI, encryptedNonce
            key_material: Contains DHPublicKey, Nonce from FIP
    ...
    async def revoke_consent(
        ...
        try:
            ...
            logger.error(f"Finvu consent revocation error: {e}")
            return AAConsentResponse(
                success=False,
                consent_id=consent_id,
                error_message=str(e),
