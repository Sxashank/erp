...
def upgrade() -> None:
    ...
    if not inspector.has_table("mst_fa_configuration"):
        op.create_table(
    ...
    op.execute(
        "CREATE UNIQUE INDEX IF NOT EXISTS ix_mst_fa_configuration_organization_id "
        "ON mst_fa_configuration (organization_id)"
    )


def downgrade() -> None:
    op.execute("DROP INDEX IF EXISTS ix_mst_fa_configuration_organization_id")
