...
export default function ESSDashboardPage() {
  ...
  const quickLinks = [
    ...
    { label: 'Payslips', icon: FileText, href: '/ess/payslips', color: 'bg-green-100 text-green-600' },
    { label: 'Reimbursements', icon: Receipt, href: '/ess/reimbursements', color: 'bg-orange-100 text-orange-600' },
    { label: 'Helpdesk', icon: HelpCircle, href: '/ess/helpdesk', color: 'bg-purple-100 text-purple-600' },
    { label: 'IT Declaration', icon: Calculator, href: '/ess/it-declaration', color: 'bg-pink-100 text-pink-600' },
    { label: 'Attendance', icon: Clock, href: '/ess/attendance', color: 'bg-cyan-100 text-cyan-600' },
  ];
            ...
            <CardContent className="space-y-4">
              <div>
                <div className="flex justify-between text-sm mb-1">
                  ...
                  <span className="font-medium">{dashboard?.leave_balance?.casual_leave || 0} days</span>
                </div>
                <Progress value={((dashboard?.leave_balance?.casual_leave || 0) / 12) * 100} className="h-2" />
              </div>
              <div>
                <div className="flex justify-between text-sm mb-1">
                  <span>Sick Leave</span>
                  <span className="font-medium">{dashboard?.leave_balance?.sick_leave || 0} days</span>
                </div>
                <Progress value={((dashboard?.leave_balance?.sick_leave || 0) / 12) * 100} className="h-2" />
              </div>
              <div>
                <div className="flex justify-between text-sm mb-1">
                  <span>Earned Leave</span>
                  <span className="font-medium">{dashboard?.leave_balance?.earned_leave || 0} days</span>
                </div>
                <Progress value={((dashboard?.leave_balance?.earned_leave || 0) / 30) * 100} className="h-2" />
              </div>
          ...
          <Card>
            ...
            <CardContent className="space-y-3">
              {(dashboard?.pending_actions?.pending_claims ?? 0) > 0 && (
                <Link to="/ess/reimbursements" className="flex items-center justify-between p-3 bg-orange-50 rounded-lg hover:bg-orange-100">
                  <div className="flex items-center gap-3">
                    <Receipt className="h-5 w-5 text-orange-600" />
                    <span className="text-sm">Pending Reimbursement Claims</span>
                  </div>
                  <Badge variant="secondary">{dashboard?.pending_actions?.pending_claims ?? 0}</Badge>
                </Link>
              )}
              {(dashboard?.pending_actions?.pending_tickets ?? 0) > 0 && (
                <Link to="/ess/helpdesk" className="flex items-center justify-between p-3 bg-purple-50 rounded-lg hover:bg-purple-100">
                  <div className="flex items-center gap-3">
                    <HelpCircle className="h-5 w-5 text-purple-600" />
                    <span className="text-sm">Open Helpdesk Tickets</span>
                  </div>
                  <Badge variant="secondary">{dashboard?.pending_actions?.pending_tickets ?? 0}</Badge>
                </Link>
              )}
              {(dashboard?.pending_actions?.pending_regularizations ?? 0) > 0 && (
                <Link to="/ess/attendance" className="flex items-center justify-between p-3 bg-cyan-50 rounded-lg hover:bg-cyan-100">
                  <div className="flex items-center gap-3">
                    <Clock className="h-5 w-5 text-cyan-600" />
                    <span className="text-sm">Pending Regularizations</span>
                  </div>
                  <Badge variant="secondary">{dashboard?.pending_actions?.pending_regularizations ?? 0}</Badge>
                </Link>
              )}
              {(dashboard?.pending_actions?.pending_declarations ?? 0) > 0 && (
                <Link to="/ess/it-declaration" className="flex items-center justify-between p-3 bg-pink-50 rounded-lg hover:bg-pink-100">
                  <div className="flex items-center gap-3">
                    <Calculator className="h-5 w-5 text-pink-600" />
                    <span className="text-sm">Complete IT Declaration</span>
                  </div>
                  <Badge variant="secondary">{dashboard?.pending_actions?.pending_declarations ?? 0}</Badge>
                </Link>
              )}
          ...
          {dashboard?.recent_payslip && (
            <Card>
              ...
              <CardContent>
                <div className="flex items-center justify-between p-4 bg-green-50 rounded-lg">
                  <div>
                    ...
                    <p className="text-2xl font-bold text-green-700">
                      {formatCurrency(dashboard.recent_payslip.net_salary)}
                    </p>
                ...
                <Link to="/ess/payslips" className="flex items-center justify-center gap-1 mt-4 text-sm text-blue-600 hover:underline">
                  View all payslips
